...
if [[ "$ENABLE_GPU" == "true" ]]; then
    ...
    VLLM_SERVE_ARGS="-c,import torch; import torch._ops; orig = torch._ops._OpNamespace.__getattr__; torch._ops._OpNamespace.__getattr__ = lambda *args: (lambda *a: None) if args[1] == 'init_cpu_memory_env' else orig(*args); from vllm.entrypoints.cli.main import main; main(),serve,/mnt/models/${MODEL_SUBDIR},--served-model-name=${SERVED_MODEL_NAME},--max-model-len=${MAX_MODEL_LEN},--api-key=${VLLM_API_KEY},--port=8080,--dtype=float32,--enforce-eager"
